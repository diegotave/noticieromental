...
> **Nota:** El repo se va a renombrar a `phonavi` en el futuro. Por ahora sigue siendo `noticieromental`.

---

## Descripción general

Sitio experimental / artístico. La home es una pantalla de videocasette con crawl del logo Phonavi animado y botones superpuestos sobre una grilla de 1920×1080 escalada. Desde ahí se navega a un carrusel de videos y al reproductor de música Phonavi CD.
...
- **Title:** Phonavi
- **Fondo:** blanco
- **Crawl:** logo `phonavi_logo_discoteca.png` repetido 8 veces, 80px de altura, animación CSS `translateX` 20s infinito
  - ⚠️ El crawl es provisorio — Diego va a reemplazarlo por una animación propia
...
> Antes se llamaba `phonavi.html` / `phonavi.css` / `phonavi.js`. Renombrado a `phonavicd.*`
...
```
PHONAVI_web/
├── phonavicd.html       ← antes phonavi.html
├── phonavicd.css        ← antes phonavi.css
├── phonavicd.js         ← antes phonavi.js
├── mini-player.css
...
### Álbumes cargados (en `phonavicd.js`)
...
- **Panel de tracks:** se abre/cierra con click en el centro del disco
...
### CSS variables clave (phonavicd.css)
```css
...
- `mini-player.css` es importado por `phonavicd.html` — contiene estilos del mini player de audio
- El botón "Acerca De" en desktop solo tiene animación hover, no navega a ningún lado
- Todas las páginas de video usan el mismo patrón CSS de flechas — candidato a refactor en archivo compartido

---

## Pendientes / ideas futuras
- [ ] Reemplazar crawl provisorio del logo por una animación propia
- [ ] Renombrar repo de `noticieromental` a `phonavi` en GitHub Settings

---

*Contexto actualizado: 21/03/2026*
